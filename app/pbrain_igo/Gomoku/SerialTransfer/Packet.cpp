...
uint8_t Packet::constructPacket(const uint16_t& messageLen, const uint8_t& packetID)
{
	...
	if (messageLen > MAX_PACKET_SIZE)
	{
		...
		length = (uint8_t)messageLen;
	}

	calcOverhead(txBuff, (uint8_t)length);
	stuffPacket(txBuff, (uint8_t)length);
	uint8_t crcVal = crc.calculate(txBuff, (uint8_t)length);

	preamble[1] = packetID;
	preamble[2] = overheadByte;
	preamble[3] = length;

	postamble = PacketBuffer + PREAMBLE_SIZE + length;
	postamble[0] = crcVal;
	postamble[1] = STOP_BYTE;
	...
	if (valid)
	{
		...
		case find_crc: ///////////////////////////////////////////
		{
			...
			else
			{
				...
				printf("calcCrc=0x%02x,recChar=0x%02x\n",calcCrc,recChar);
