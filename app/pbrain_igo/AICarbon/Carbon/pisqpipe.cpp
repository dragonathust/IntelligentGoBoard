...
static struct Event event1;
static struct Event event2;

void CreateEvent(struct Event *p, bool bManualReset, bool bInitialState)
{
	//p->mutex = PTHREAD_MUTEX_INITIALIZER;
	pthread_mutex_init(&p->mutex, NULL);
	//p->cvar = PTHREAD_COND_INITIALIZER;
	pthread_cond_init(&p->cvar, NULL);
	p->bManualReset = bManualReset;
	p->bState = bInitialState;
}

void SetEvent(struct Event *p)
{
...
void WaitForEvent(struct Event *p)
{
    pthread_mutex_lock(&p->mutex);
    while (!p->bState) {
...
void ResetEvent(struct Event *p)
{
	if(p->bManualReset) {	
    pthread_mutex_lock(&p->mutex);
    p->bState = false;
...
#else
	printf("MESSAGE Gomoku AI should not be started directly. Please install gomoku manager (http://sourceforge.net/projects/piskvork). Then enter path to this exe file in players settings.\n");
#endif
...
#else
	pthread_t tid;
	CreateEvent(&event1, false, false);
	pthread_create(&tid,NULL,threadLoop,NULL); 
	CreateEvent(&event2, true, true);
#endif	
	for(;;){
		get_line();
		do_command();
