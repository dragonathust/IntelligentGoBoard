...
static struct gtp_command commands[] = {
  ...
  {"showboard_raw",        	      gtp_showboard_raw},
...
raw_showboard(FILE *outfile)
{
  ...
  unsigned int board_display_buffer[board_size * 2];
  unsigned int board_display_send[board_size * 2];
  char buf[256];
  ...
  for (i = 0; i < board_size; i++) {
    for (j = 0; j < board_size; j++) {
      if (BOARD(i, j) != EMPTY) {
	    board_display_buffer[i] |= 1 << j;
	  }
	  
	  if (BOARD(i, j) == WHITE) {
	    board_display_buffer[board_size + i] |= 1 << j;
	  }
    }
  }

  for ( i = 0; i < board_size * 2; i++ ) {  
     line = board_display_buffer[i];
  ...
  snprintf(buf, sizeof(board_display_send)/2 + 1 ,"%s", (char*)&board_display_send[0]);
  fprintf(outfile, "%s", buf);
  snprintf(buf, sizeof(board_display_send)/2 + 1 ,"%s", (char*)&board_display_send[board_size]);
  fprintf(outfile, "%s\n", buf);
}
 ...
 */
static int
gtp_showboard_raw(char *s)
{
  UNUSED(s);

  gtp_start_response(GTP_SUCCESS);
  raw_showboard(gtp_output_file);
  return gtp_finish_response();
}
/*****************************************************************************/
